...
pid_t get_fgpid(struct job_t * jobs);
...
void eval(char *cmdline) 
{
  ...
  int fds[2]; 
  pipe(fds); 
  // only block SIGCHLD, because child to shell is only SIGCHLD
  ...
  if(!builtin_cmd(argv1)){
    ...
    if(cmd2 != NULL){
      setpgid(pid2 ,pid); //set child2's pgid to child1's pgid
      close(fds[0]); 
      close(fds[1]);
      addjob(jobs, pid2, bg?BG:FG, cmdline);
    }
    //unblock the signal after adding jobs
 ...
 */
int parseline(const char *cmdline, char **argv, int cmdnums) 
{
...
void waitfg(pid_t pid)
{
  sigset_t empty;//less strict      
  sigemptyset(&empty);
  while (get_fgpid(jobs) == pid){
      sigsuspend(&empty);
  }
  return; 
}
...
/*get_fgpid - Returns the forground pid if it exists, otherwise returns 0. */
pid_t get_fgpid(struct job_t * jobs){
  int i; 
  for(i = 0; i< MAXJOBS; i++){
    if(jobs[i].state == FG)
